...
# --- Indstillinger ---
candidates = ["Anna", "Bjørn", "Clara", "David", "Elias"]
sheet_name = "Bestyrelsesvalg 2025"
...
def has_already_voted(pseudonym):
    sheet = get_sheet()
    records = sheet.col_values(1)  # Kolonne A = pseudonymer
    return pseudonym in records

# --- Funktion: Gem stemmer ---
def submit_vote_to_sheet(pseudonym, votes):
    sheet = get_sheet()
    row = [pseudonym] + votes + [""] * (4 - len(votes))  # Fyld op til 4 stemmer
    sheet.append_row(row)

# --- Start App ---
st.title("🗳️ Bestyrelsesvalg 2025")
st.write("Velkommen til den digitale afstemning for valg til bestyrelsen! 👋")
...
st.write("Vælg op til **4 kandidater**, som du ønsker skal repræsentere dig i bestyrelsen. 👇")

if "confirmed" not in st.session_state:
    st.session_state["confirmed"] = False

selection = st.multiselect("🎯 Vælg dine kandidater", candidates, max_selections=4)
